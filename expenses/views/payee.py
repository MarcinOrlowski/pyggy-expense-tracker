...
def payee_create(request):
    ...
    context = {"form": form, "title": "Create New Payee"}
...
def payee_edit(request, pk):
    ...
    context = {"form": form, "payee": payee, "title": f"Edit Payee: {payee.name}"}
    return render(request, "expenses/payee_form.html", context)
...
def payee_delete(request, pk):
    ...
    if request.method == "POST":
        if expense_count > 0:
            messages.error(
                request,
                f'Cannot delete payee "{payee.name}" because it has {expense_count} associated expenses.',
            )
        else:
            name = payee.name
            payee.delete()
