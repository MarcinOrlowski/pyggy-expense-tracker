...
class ExpenseEditingPermissionsTest(TestCase):
    ...
    def test_get_edit_restrictions_multiple_reasons(self):
        ...
        if isinstance(reasons, list):
            self.assertIn(
                "Amount cannot be edited because expense has paid items", reasons
            )
    ...
    def test_expense_edit_view_redirects_for_non_editable(self):
        ...
        url = reverse(
            "expense_edit", kwargs={"budget_id": self.budget.id, "pk": expense.pk}
        )
        ...
        expected_url = reverse(
            "expense_detail", kwargs={"budget_id": self.budget.id, "pk": expense.pk}
        )
