...
class SectionContextProcessorTest(TestCase):
    ...
    def test_dashboard_section_detection(self):
        ...
        self.assertEqual(
            self._get_section_class("/budgets/1/dashboard/"), "section-dashboard"
        )

    def test_expenses_section_detection(self):
        """Test that expense-related URLs are correctly identified."""
        self.assertEqual(
            self._get_section_class("/budgets/1/expenses/"), "section-expenses"
        )
        self.assertEqual(
            self._get_section_class("/budgets/1/expenses/create/"), "section-expenses"
        )

    def test_months_section_detection(self):
        """Test that month-related URLs are correctly identified."""
        self.assertEqual(
            self._get_section_class("/budgets/1/months/"), "section-months"
        )
        self.assertEqual(
            self._get_section_class("/budgets/1/months/2024/1/"), "section-months"
        )

    def test_payments_section_detection(self):
        """Test that payment-related URLs are correctly identified."""
        self.assertEqual(
            self._get_section_class("/budgets/1/expense-items/1/pay/"),
            "section-payments",
        )
        self.assertEqual(
            self._get_section_class("/budgets/1/expense-items/1/unpay/"),
            "section-payments",
        )
        self.assertEqual(
            self._get_section_class("/budgets/1/expense-items/1/payments/"),
            "section-payments",
        )
    ...
    def test_payment_methods_section_detection(self):
        ...
        self.assertEqual(
            self._get_section_class("/payment-methods/"), "section-payment-methods"
        )
        self.assertEqual(
            self._get_section_class("/payment-methods/create/"),
    ...
    def test_help_section_detection(self):
        ...
        self.assertEqual(
            self._get_section_class("/help/getting_started/"), "section-help"
        )
