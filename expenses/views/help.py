...
def help_index(request):
    ...
    if not os.path.exists(docs_dir):
        return render(
            request,
            "expenses/help_index.html",
            {"docs": [], "error": "Documentation directory not found."},
        )
    ...
    try:
        ...
        return render(
            request,
            "expenses/help_index.html",
            {"docs": [], "error": f"Error reading documentation directory: {e}"},
        )
...
def help_page(request, page_name):
    ...
    try:
        ...
        if not real_file_path.startswith(real_docs_dir):
            return _render_help_error(
                request,
                page_name,
                "Documentation page not found",
                "The requested documentation page could not be found or is not accessible.",
            )
    ...
    if not os.path.exists(file_path) or not os.path.isfile(file_path):
        return _render_help_error(
            ...
            page_name,
            "Documentation page not found",
            f"The documentation page '{page_name}' does not exist.",
        )
    ...
    try:
        ...
        return render(
            request,
            "expenses/help_page.html",
            {"title": title, "content": html_content, "page_name": page_name},
        )
        ...
        return _render_help_error(
            ...
            f"Unable to read the documentation file: {e}",
        )
