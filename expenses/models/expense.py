...
class Expense(models.Model):
    ...
    def clean(self) -> None:
        ...
        if (
            self.expense_type
            in [
                self.TYPE_ENDLESS_RECURRING,
                self.TYPE_ONE_TIME,
                self.TYPE_RECURRING_WITH_END,
            ]
            and self.total_parts > 0
        ):
        ...
        if self.start_date and self.budget_id:
            ...
            if most_recent_month:
                ...
                if self.start_date < current_month_start:
                    raise ValidationError(
                        f"Start date cannot be earlier than the current month ({most_recent_month})"
                    )
            ...
            if most_recent_month:
                ...
                if most_recent_month.month == 12:
                    ...
                    next_month_start = date(
                        most_recent_month.year, most_recent_month.month + 1, 1
                    )
